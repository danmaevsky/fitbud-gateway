...
router.get("/:workoutId", util.AuthTokenMiddleware, async (request, response) => {
    ...
    if (fitnessStatus !== 200) {
        return response.status(fitnessStatus).send(fitnessResponse);
    }

    let token = request.get("Authorization").split(" ")[1];
    let userId = jwt.decode(token).userId;
    if (!userId || userId !== fitnessResponse.userId) {
        console.log(`userId in accessToken (${userId}) does not match userId in query parameters (${request.query.userId})`);
        return response.status(401).send({ message: "Not permitted to view workouts that do not belong to you!" });
    }
